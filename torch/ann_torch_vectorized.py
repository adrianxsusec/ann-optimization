...
def predict(Theta1, Theta2, X):
    ...
    if use_torch:
        ...
        Theta1_torch = torch.empty(Theta1.shape, dtype=torch.float64, device=device)
        Theta2_torch = torch.empty(Theta2.shape, dtype=torch.float64, device=device)

        # Copy data directly to pre-allocated tensors
        X_torch.copy_(torch.from_numpy(X).cuda(device=device))
        Theta1_torch = torch.tensor(Theta1, dtype=torch.float64, requires_grad=True).cuda(device=device)
        Theta2_torch = torch.tensor(Theta2, dtype=torch.float64, requires_grad=True).cuda(device=device)

        m = X_torch.shape[0]
        a1 = torch.cat([torch.ones(m, 1).to(device), X_torch], dim=1)
        a2 = torch.sigmoid(a1 @ Theta1_torch.T)
        a2 = torch.cat([torch.ones(m, 1).to(device), a2], dim=1)
        a3 = torch.sigmoid(a2 @ Theta2_torch.T)
...
def cost_function(theta, input_layer_size, hidden_layer_size, num_labels, X, y, lmbda):
    ...
    if use_torch:
        ...
        # Add regularization
        J += lmbda / (2 * m) * (torch.sum(Theta1_torch[:, 1:] ** 2) + torch.sum(Theta2_torch[:, 1:] ** 2))
...
def gradient(theta, input_layer_size, hidden_layer_size, num_labels, X, y, lmbda):
    ...
    if use_torch:
        ...
        Theta1, Theta2 = reshape(theta, input_layer_size, hidden_layer_size, num_labels)
        Theta1_torch = theta_torch[:hidden_layer_size * (input_layer_size + 1)].view(hidden_layer_size,
                                                                                     input_layer_size + 1)
        Theta2_torch = theta_torch[hidden_layer_size * (input_layer_size + 1):].view(num_labels, hidden_layer_size + 1)

        # Number of training values
        m = len(y)

        # Initialize gradients
        Delta1 = torch.zeros((hidden_layer_size, input_layer_size + 1)).to(device)
        Delta2 = torch.zeros((num_labels, hidden_layer_size + 1)).to(device)
        _Delta1 = np.zeros((hidden_layer_size, input_layer_size + 1))
        _Delta2 = np.zeros((num_labels, hidden_layer_size + 1))
        # Forward pass and backpropagation

        X_batch = X_torch.reshape(m, input_layer_size)  # Reshape tensor
        X_batch = torch.cat([torch.ones(m, 1, device=device), X_batch], dim=1)  # Add bias

        # Forward pass
        z2 = Theta1_torch @ X_batch.T  # Matrix multiplication
        a2 = torch.sigmoid(z2)  # Activation function
        a2 = torch.cat([torch.ones(1, m, device=device), a2], dim=0)  # Add bias
        a3 = torch.sigmoid(Theta2_torch @ a2)  # Output layer

        # One-hot encoding
        y_onehot = torch.zeros(num_labels, m, device=device)
        y_onehot.scatter_(0, y_torch.long(), 1)

        # Backpropagation
        delta3 = a3 - y_onehot
        Delta2 += delta3 @ a2.T

        delta2 = (Theta2_torch[:, 1:].T @ delta3) * torch_grad_g(z2)
        Delta1 += delta2 @ X_batch.T
        # Average gradients
        Theta1_grad = Delta1 / m
        Theta2_grad = Delta2 / m

        # Add regularization
        Theta1_grad[:, 1:] += (lmbda / m) * Theta1_torch[:, 1:]
        Theta2_grad[:, 1:] += (lmbda / m) * Theta2_torch[:, 1:]
