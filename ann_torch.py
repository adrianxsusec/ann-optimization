...
def gradient(theta, input_layer_size, hidden_layer_size, num_labels, X, y, lmbda):
    ...
    if use_torch:
        ...
        for t in range(m):
            # Forward pass
            _a1 = X[t, :].reshape((input_layer_size, 1))
            _a1 = np.vstack((1, _a1))  # +bias



            a1 = X_torch[t].view(-1, 1)
            a1 = torch.cat([torch.ones(1, 1).to(device), a1])  # Add bias

            _z2 = Theta1 @ _a1
            _a2 = g(_z2)
            _a2 = np.vstack((1, _a2))  # +bias
            _a3 = g(Theta2 @ _a2)

            z2 = Theta1_torch @ a1
            a2 = torch.sigmoid(z2)
            a2 = torch.cat([torch.ones(1, 1).to(device), a2])  # Add bias
            a3 = torch.sigmoid(Theta2_torch @ a2)


            _y_k = np.zeros((num_labels, 1))
            _y_k[y[t, 0].astype(int)] = 1
            _delta3 = _a3 - _y_k
            _Delta2 += (_delta3 @ _a2.T)

            # Compute error for layer 3
            y_k = torch.zeros(num_labels, 1).to(device)
            y_k[y_torch[t].to(torch.int)] = 1
            delta3 = a3 - y_k
            Delta2 += delta3 @ a2.T


            _delta2 = (Theta2[:, 1:].T @ _delta3) * grad_g(_z2)
            _Delta1 += (_delta2 @ _a1.T)
